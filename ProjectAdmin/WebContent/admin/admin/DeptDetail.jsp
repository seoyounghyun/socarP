   ...
   <link rel="shortcut icon" href="${pageContext.request.contextPath}/bootstrap/img/favicon.png"/>
	...
	<script>
		var goList = function(){
			...
			location.href="<c:url value='/Admin/DeptList.do' />";
		};		
		function goEdit() {
				location.href="<c:url value='/Admin/EditDept.do' />?dept_no=${dto.dept_no}";	
		}
		function goDelete() {
			if(confirm("정말 삭제하시겠습니까?"))
			location.href="<c:url value='/Admin/DeleteDept.do' />?dept_no=${dto.dept_no}";
		}
	</script>
				  ...
				  <h2>부서 입력</h2><br/><br/>
				      	  ...
				      	  <tbody>
					      	  ...
					      	  <tr>
					      	  		...
					      	  		<td>
								      	<span>${dto.dept_no}</span>
					      	  		</td>
					      	  </tr>
					      	  <tr>
					      	  		<td><label>부서명</label></td>
					      	  		<td>
										<span>${dto.dept_name}</span>
									</td>
					      	  </tr>
					      	  <tr>
								    ...
								    <td>
					      	  			<span>${dto.dept_desc}</span>
								    </td>
					      	  </tr>
					      	  <tr>
					      	  		<td><label>부서등록일</label></td>
					      	  		<td>
										<span>${dto.dept_regidate}</span>
									</td>
					      	  </tr>
				      ...
				      <button type="submit" class="btn btn-info" onclick="goEdit()">수정하기</button>
				      <button type="submit" class="btn btn-info" onclick="goDelete()">삭제하기</button>
				      <button type="button" class="btn btn-info" onclick="goList()">리스트가기</button>
